...
pub const PUNCTUATIONS: [char; 10] = [';', ':', ',', '(', ')', '{', '}', '[', ']', '"'];
...
impl Tokenizer {
    ...
    pub fn tokenize(&mut self) -> Vec<Tokens> {
        ...
        while self.peek() != '\0' {
            while !self.current_char.is_whitespace() && !PUNCTUATIONS.contains(&self.current_char) {
                buf.push(self.current_char);
                self.next_char();
            }

            if buf.is_empty() && !PUNCTUATIONS.contains(&self.current_char) {
                self.next_char();
                continue;
            };
            ...
            if KEYWORDS.contains(&buf.as_str()) {
                match buf.as_str() {
                    "let" => self.tokens.push(Tokens::Let),
                        ...
                        while self.current_char != '"' {
                            ...
                            if self.current_char == '\\' {
                                ...
                                match self.current_char {
                                    ...
                                    '"' => buf.push('"'),
                                    '\'' => buf.push('\''),
                                    _ => panic!("Unknown escape character: {}", self.current_char),
                                }
                            }
                        }
                        self.tokens
